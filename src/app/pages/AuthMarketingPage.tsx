...
import {
  CheckCircle2, XCircle, ShieldCheck, Fingerprint, Copy,
  Play, Pause, Plus, ToggleLeft, ToggleRight, ChevronDown, ChevronRight,
  Undo2, Bell, Webhook, Settings2, Filter,
} from "lucide-react";
import { cn } from "@/demo/lib/utils";
import { MY_PROFILE, MY_CAMPAIGNS, type IncomingLead } from "@/demo/crm/data/mockData";
import { useIncomingLeads, useAcceptLead, useRejectLead, useReturnLead } from "../hooks/useSupabaseData";
...
export const AuthMarketingPage = () => {
...
const IncomingLeadsTab = () => {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground py-4">Loading leads...</div>;
  }

  if (leads.length === 0) {
    return (
      <div className="bg-background rounded-lg border border-border p-8 text-center">
        <p className="text-muted-foreground text-sm">No incoming leads yet. Leads will appear here as they come in.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto">
        {([
          ...
          { id: "rejected" as const, label: "Rejected", count: leads.filter((l) => l.status === "rejected").length },
          { id: "returned" as const, label: "Returned", count: leads.filter((l) => l.status === "returned").length },
        ]).map((f) => (
          <button
            key={f.id}
...
const LeadActions = ({ lead }: { lead: IncomingLead }) => {
  ...
  const returnLead = useReturnLead();

  if (lead.status === "new") {
    return (
      <div className="flex items-center gap-1.5">
        <button
          onClick={() => acceptLead.mutate(lead)}
          disabled={acceptLead.isPending}
          className="h-7 px-2.5 rounded-md bg-foreground text-background text-xs font-semibold flex items-center gap-1 hover:opacity-80 transition-opacity disabled:opacity-50"
        >
          <CheckCircle2 className="h-3 w-3" /> {acceptLead.isPending ? "..." : "Accept"}
        </button>
        <button
          onClick={() => rejectLead.mutate(lead.id)}
          disabled={rejectLead.isPending}
          className="h-7 px-2.5 rounded-md border border-border text-xs font-medium flex items-center gap-1 hover:bg-muted transition-colors disabled:opacity-50"
        >
  ...
  if (lead.status === "accepted") {
    return (
      <button
        onClick={() => returnLead.mutate(lead.id)}
      ...
      <div key={campaign.id} className="bg-background rounded-lg border border-border p-4 md:p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {campaign.filters.map((f) => (
            <span key={f} className="px-2 py-0.5 bg-muted rounded text-[11px] font-medium text-muted-foreground">{f}</span>
          ))}
        </div>
...
/* ── Analytics Tab ── */
const AnalyticsTab = () => {
  const { data: leads = [] } = useIncomingLeads();
...
/* ── Settings Tab (static, same as demo) ── */
...
const MOCK_WEBHOOKS = [
  { url: "https://crm.example.io/webhook/new-lead", event: "Lead Accepted", active: true },
  { url: "https://crm.example.io/webhook/returns", event: "Lead Returned", active: true },
];
  ...
  <div className="space-y-4">
    <div className="bg-background rounded-lg border border-border p-4 md:p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="h-8 px-3 rounded-lg border border-border text-xs font-medium hover:bg-muted transition-colors">Edit Preferences</button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm">
        <div>
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">States</p>
          <div className="flex flex-wrap gap-1">
            {MY_PROFILE.states.map((s) => <span key={s} className="px-2 py-0.5 bg-muted rounded text-xs font-medium text-foreground">{s}</span>)}
          </div>
        </div>
        <div>
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">Loan Types</p>
          <div className="flex flex-wrap gap-1">
            {MY_PROFILE.loanTypes.map((lt) => <span key={lt} className="px-2 py-0.5 bg-muted rounded text-xs font-medium text-foreground">{lt}</span>)}
          </div>
        </div>
        <div>
        ...
        {VALIDATION_SERVICES.map((service) => (
          <div key={service.name} className="flex items-center justify-between px-4 md:px-5 py-4">
            ...
            <div className="shrink-0 ml-3">
              {service.enabled ? <ToggleRight className="h-6 w-6 text-foreground" /> : <ToggleLeft className="h-6 w-6 text-muted-foreground" />}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {MOCK_WEBHOOKS.map((wh) => (
          <div key={wh.url} className="flex items-center justify-between bg-muted/50 rounded-lg px-3 py-2.5">
            ...
            <div className="shrink-0 ml-3">
              {wh.active ? <ToggleRight className="h-5 w-5 text-foreground" /> : <ToggleLeft className="h-5 w-5 text-muted-foreground" />}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {ALERT_SETTINGS.map((alert) => (
          <div key={alert.label} className="flex items-center justify-between">
            ...
            <div className="shrink-0 ml-3">
              {alert.enabled ? <ToggleRight className="h-5 w-5 text-foreground" /> : <ToggleLeft className="h-5 w-5 text-muted-foreground" />}
            </div>
          </div>
        ))}
      </div>
