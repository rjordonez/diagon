    ...
    <>
      ...
      <div className="h-screen w-screen flex flex-col overflow-hidden">
        <div className="flex flex-1 min-h-0">
          {/* Mobile overlay — must be before sidebar in DOM */}
          {sidebarOpen && (
          ...
          <nav className={cn(
            ...
            <div className="h-[56px] shrink-0 px-4 border-b border-border flex items-center justify-between">
              <div>
              ...
              {NAV_ITEMS.map((item) => (
                <NavLink key={item.to} to={item.to} end={item.end} onClick={() => setSidebarOpen(false)}
                  className={({ isActive }) => cn(
                    ...
                    isActive ? "bg-muted text-foreground font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                  )}>
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            {/* Top Bar — fixed 56px */}
            <header className="h-[56px] shrink-0 bg-background border-b border-border flex items-center justify-between px-4 md:px-6">
      ...
      {/* Copilot Chat — outside layout to avoid overflow-hidden issues */}
      <CopilotChat />
    </>
  );
};
